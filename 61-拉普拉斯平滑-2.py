...
def backward_euler_smooth(verts, adj_mat, steps=50, lam=0.1):
    v = verts.copy()
    ...
    for _ in tqdm(range(steps)):
        ...
        for d in range(3):
            v_new[:, d] = spla.spsolve(A, v[:, d])

        lap = np.asarray(L @ v)
        history.append(v_new.copy())
        lap_history.append(lap.copy())

        v = v_new

    return history, lap_history


def precompute_all(verts, faces, adj_mat, steps=10, lam=0.5):
    print("Precomputing smoothing...")
    ...
    for _ in tqdm(range(steps)):
        ...
        lap = np.asarray(L @ v_new)

        verts_history.append(v_new.copy())
        lap_history.append(lap.copy())
    ...
    for t in range(len(verts_history)):
        verts_t = verts_history[t]
        laps_t  = lap_history[t]

        vertex_normals = get_vertex_normals(verts_t, faces)
        norm_L = np.linalg.norm(laps_t, axis=1)
        sign = np.sign(np.sum(laps_t * vertex_normals, axis=1))
        scale = norm_L * sign

        max_val = np.abs(scale).max()
        if max_val < 1e-8:
            normed = scale
        else:
            normed = scale / max_val

        colors = np.array([blue_gray_red(s) for s in normed], dtype=np.float32)

        # ✔ 展开三角形
        data = []
        for f in faces:
            n = get_normal(verts_t, *f)
            for idx in f:
                data.extend([*verts_t[idx], *n, *colors[idx]])
...
all_data = precompute_all(verts, faces, adj_mat, steps=10, lam=0.5)
...
# ====== 初始化 ======
def init():
    global shader, VAO, VBOs, vertex_count
...
def display():
    ...
    glBindVertexArray(VAO)
    glBindBuffer(GL_ARRAY_BUFFER, VBOs[time_step])
...
def mouse(button, state, x, y):
    global left_down, right_down, last_x, last_y, last_x_light, last_y_light, time_step
    ...
    if button == 3:  # wheel down
        time_step = max(0, time_step - 1)
        glutPostRedisplay()
    elif button == 4:  # wheel up
        time_step = min(len(all_data) - 1, time_step + 1)
        glutPostRedisplay()
